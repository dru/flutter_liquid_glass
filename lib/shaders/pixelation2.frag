...
void main() {
    vec3 iMouse = vec3(uPixels, uPixels, 1.0);
    ...
    vec2 M  = iMouse.z > 0.0 ? (iMouse.xy - 0.5 * R) / R.y : vec2(0.);
    ...
    vec3 col = mix(texture(uTexture, uv).rgb * 0.8, 0.1 + LiquidGlass(uTexture, uv2, 10.0, 10.0, 20.0).rgb * 0.7, icon.x);
